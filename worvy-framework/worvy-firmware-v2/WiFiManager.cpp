...
void WiFiManager::setupConfigPortal()
{
  ...
  server->on(String(F("/ping")).c_str(), std::bind(&WiFiManager::handlePing, this));
...
void WiFiManager::handlePing()
{
  DEBUG_WM(F("Handle ping"));
  if (captivePortal())
  { // If caprive portal redirect instead of displaying the page.
    return;
  }
...
bool WiFiManager::saveConfig(String body)
{
  ...
  char body_c[body.length() + 1];
  strcpy(body_c, body.c_str());
  configFile.print(body_c);
  configFile.close();
  // serializeJson(body_c, configFile);
  return true;
}
